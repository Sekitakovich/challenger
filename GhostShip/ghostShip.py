...
class GhostShip(object):
    ...
    def GGA(self) -> bytes:
        item = ['GPGGA', self.at.strftime(self.hmsFormatN),
    ...
    def GLL(self) -> bytes:
        item = ['GPGLL',
                round(self.location.lat, 4), self.ns, round(self.location.lng, 4), self.ew,
                self.at.strftime(self.hmsFormatN), self.isValid, self.statS]
        return self.toNMEA(src=item)

    def RMC(self) -> bytes:
        item = ['GPRMC', self.at.strftime(self.hmsFormatN), self.isValid,
                round(self.location.lat, 4), self.ns, round(self.location.lng, 4), self.ew,
                self.spd, self.hdg, self.at.strftime(self.ymdFormat), self.md, self.ew]
        return self.toNMEA(src=item)

    def ZDA(self) -> bytes:
        item = ['GPZDA',
                self.at.strftime(self.hmsFormatP)[:-4],
